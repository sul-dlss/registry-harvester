...
if [ $? -eq 0 ] ; then
        ...
        if [ $? -eq 0 ] ; then
	  e_msg="$APP_NAME uat BLOCKED, already running, found pid $PID $t_stamp"
...
while [ true ] ; do 
  ...
  t_stamp=`date "$dfmt"`
  echo "$t_stamp $APP_NAME exit to harness, exit status $EXIT_CODE error count: $errcnt" >> $HARNESS_LOG



  echo "$t_stamp $APP_NAME sleeping for $sleeptime"   >> $HARNESS_LOG     
  /bin/sleep $sleeptime      
